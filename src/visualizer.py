...
class Visualizer:
    ...
    def plot_clustered_data(temperatures, cluster_numbers,
        ...
        center_temperatures = sorted([center[0] for
                                      center in cluster_centers])
        for i in range(len(center_temperatures) - 1):
            boundary_temp = (center_temperatures[i] +
                             center_temperatures[i + 1]) / 2
            plt.axhline(y=boundary_temp, linestyle='--',
                        color='gray', alpha=0.5)

        # labels the x-axis with each point showing the date & city
        plt.xticks(ticks=range(len(sorted_labels)),
                   labels=sorted_labels, rotation=90)
    ...
    def plot_humidity_predictions(actual, predicted, x_labels):
        ...
        plt.plot(x, actual, label='Actual Humidity',
                 marker='o')
        plt.plot(x, predicted, label='Predicted Humidity',
                 marker='x')

        # creates the labels & point titles
        plt.xlabel("Date" if x_labels else
                   "Sample Index")
        plt.ylabel("Humidity (%)")
